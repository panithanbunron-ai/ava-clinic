...
<template>
    <div class="p-4 md:p-8 max-w-6xl mx-auto min-h-screen bg-slate-50/50">
        <!-- Main Form Container -->
        <div class="bg-white rounded-3xl shadow-xl shadow-blue-900/5 ring-1 ring-gray-100 flex flex-col h-full overflow-hidden transition-all duration-300">

            <!-- Dynamic Header with Gradient -->
            <div class="px-8 py-8 border-b border-gray-100 flex justify-between items-start bg-gradient-to-b from-blue-50/50 to-white relative overflow-hidden">
                <div class="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
                <div class="absolute bottom-0 left-0 w-48 h-48 bg-indigo-500/5 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4"></div>

                <div class="flex items-center gap-5 relative z-10">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center shrink-0 shadow-lg shadow-blue-500/20 transform transition-transform hover:scale-105 duration-300">
                        <UIcon v-if="activeTab === 1" name="i-heroicons-user-plus" class="w-7 h-7" />
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1.5 font-medium flex items-center gap-2">
                            ...
                            <template v-else>
                                <span class="text-blue-600 font-semibold bg-blue-50 px-2 py-0.5 rounded-md">CN-00184</span>
                                <span class="text-gray-300">•</span> 35 ปี <span class="text-gray-300">•</span> {{ form.gender }}
                            </template>
                        </p>
                    </div>
                </div>
            ...
            <div class="px-8 bg-white border-b border-gray-100/80">
                <div class="flex gap-2">
                    ...
                    >
                        ...
                        <div v-if="tab.id < 4" class="absolute right-0 top-1/2 -translate-y-1/2 w-8 h-[2px] bg-gray-100 -mr-4 z-0"></div>
                        ...
                        <div
                            ...
                            :class="[
                                activeTab === tab.id ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/30 scale-110' :
                                activeTab > tab.id ? 'bg-emerald-500 text-white shadow-md shadow-emerald-500/20' : 'bg-gray-50 text-gray-400 ring-1 ring-gray-200 group-hover:bg-gray-100 group-hover:text-gray-600'
                            ]"
                        >
                            <UIcon v-if="activeTab > tab.id" name="i-heroicons-check-16-solid" class="w-5 h-5" />
                            <span v-else>{{ tab.id }}</span>

                            <!-- Pulsing ring for active tab -->
                            <div v-if="activeTab === tab.id" class="absolute -inset-1.5 rounded-full border border-blue-400/30 animate-pulse"></div>
                        </div>

                        <!-- Text -->
                        <div class="flex flex-col z-10">
                            <span class="text-sm font-bold transition-colors duration-200"
                                :class="activeTab === tab.id ? 'text-gray-900' : (activeTab > tab.id ? 'text-emerald-700' : 'text-gray-500 group-hover:text-gray-700')"
                            >
                                {{ tab.name }}
                            </span>
                            <span v-if="tab.suffix" class="text-[11px] font-medium text-gray-400">{{ tab.suffix }}</span>
                        </div>

                        <!-- Red dot for required in Tab 1 -->
                        <div v-if="tab.id === 1 && activeTab === 1" class="absolute top-4 right-1/4 w-2 h-2 rounded-full bg-red-500 shadow-sm shadow-red-500/50"></div>

                        <!-- Active Bottom Line Indicator -->
                        <div v-if="activeTab === tab.id" class="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-t-full"></div>
                    </div>
                </div>
            </div>
            ...
            <!-- Refined Footer -->
            <div class="px-8 py-5 border-t border-gray-100 flex items-center justify-between bg-white/80 backdrop-blur-md rounded-b-3xl">
                <!-- Progress Bar -->
                <div class="flex items-center gap-5">
                    <div class="flex gap-2">
                        <div v-for="i in 4" :key="i" class="h-2 rounded-full transition-all duration-500" :class="[
                            i === activeTab ? 'w-10 bg-gradient-to-r from-blue-600 to-indigo-600 shadow-sm shadow-blue-500/20' :
                            i < activeTab ? 'w-6 bg-emerald-400' : 'w-6 bg-gray-100'
                        ]"></div>
                    </div>
                    <div class="text-sm text-gray-500 font-bold tracking-wide">ขั้นตอนที่ <span class="text-gray-900">{{ activeTab }}</span> / 4</div>
                </div>
